...
export default class RR extends Component {
  ...
  checkmax()
  {
    ...
    var max=0
    for(i=0;i<size;i++)
    { if(data[i].arvtime>max){max = data[i].arvtime;} }
    f=f+max;
    return;
  }
  addready(c)
  ...
  process(c)
  {
      ...
      else if(Quantum_count>=Quantum && ready.length!=0)
      {
      ...
      else if((runing[0].remaintime==0 && ready.length==0)|| (Quantum_count>=Quantum && ready.length==0))
      { 
        if(data.length==0)
        ...
        <View style={styles.main_body}>
          <View >
              ...
              <Text style={styles.t}>Enter Process ID: </Text>
                ...
                <Text style={styles.t}>Time Quantum: (should be same for all process): </Text>
                <TextInput style = {styles.in_area} keyboardType={'numeric'} onChangeText={this.handletimequantum}></TextInput>
